...
>
	{#each projects as currentProject, i (i)}
		<div class="snap-center shrink-0 h-40 md:h-60 lg:h-80 aspect-video">
			<Flipin delay={i * 0.1}>
				<button
					class="flex w-full h-full flex-col border-y-[3px] border-secondary border-double rounded-xl transition-transform hover:scale-105 duration-75 shadow-xl"
					on:click={() => {
				...
				>
					...
					<div
						class="transparent-gradient-fade absolute-centered bg-black bg-opacity-80 w-full h-fit py-5 border-y-[2px] border-secondary"
					>
